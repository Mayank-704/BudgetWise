...
export async function PATCH(req: NextRequest, context: { params: Promise<{ workspaceId: string, pageId: string, transactionId: string }> }) {
  await dbConnect();
...
export async function DELETE(req: NextRequest, context: { params: Promise<{ workspaceId: string, pageId: string, transactionId: string }> }) {
  await dbConnect();
  const { workspaceId, pageId, transactionId } = await context.params;
  const workspace = await Workspace.findById(workspaceId);
  if (!workspace) return NextResponse.json({ error: "Workspace not found" }, { status: 404 });
